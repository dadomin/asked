...
public class LoginController extends MasterController {
	...
	private UserVO checkLogin(String id, String pw) {
		...
		String sql = "SELECT * FROM `ask_user` WHERE id = ? AND pass = PASSWORD(?)";
		...
		try {
			...
			Util.showAlert("에러", "데이터베이스 처리중 오류발생, 인터넷 연결을 확인하세요", AlertType.ERROR);
			return null;
		} finally {
			JDBCUtil.close(rs);
	...
	public void registerPage() {
		...
		RegisterController mc = (RegisterController)MainApp.app.controllerMap.get("register");
		mc.clearInput();
	}
}
